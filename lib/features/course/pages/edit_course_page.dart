...
class _EditCoursePageState extends State<EditCoursePage> {
    ...
    return BlocConsumer<CourseCubit, CourseState>(
      listenWhen: (previous, current) {
        if (current is CourseSuccess &&
            current.routeName == AppRouter.editCourse) {
          if (current.actionMethod == ActionMethod.updating ||
              current.actionMethod == ActionMethod.fetching) {
            return true;
          }
          if (current.actionMethod == ActionMethod.updated ||
              current.actionMethod == ActionMethod.fetched) {
            return true;
          }
          if (current.actionMethod == ActionMethod.notUpdated ||
              current.actionMethod == ActionMethod.notFetched) {
            return true;
          }
          return false;
        }
      ...
      listener: (context, state) {
        if (state is CourseSuccess) {
          if (state.actionMethod == ActionMethod.updating) {
            MevTechUtilities.showProgressIndicator(context);
          } else if (state.actionMethod == ActionMethod.notUpdated ||
          ...
          } else if (state.actionMethod == ActionMethod.updated) {
            MevTechUtilities.hideProgressIndicator(context);
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            dropdownBtn(
                              value: spCategory,
                              hint: state.categories.isNotEmpty
                        ...
                        //         onPressed: courseCubit.testGetCourse,
                        //         child: Text('Test'))),
                      ],
                    ),
                  ),
  ...
  //     color: AppColor.primaryLight1,
  //     child: DropdownButtonHideUnderline(
  //       child: DropdownButton<CourseCategoryModel>(
  //         isDense: true,
  //         padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
  //         hint: Text(hint ?? ''),
  //         menuMaxHeight: MediaQuery.of(context).size.height * 0.5,
  //         value: value,
  //         items: items.map((String value) {
  //           return DropdownMenuItem<CourseCategoryModel>(
  //             value: value,
  //             child: Text(
  //               value,
  //               style: TextStyle(fontSize: 13.sp),
                                    ...
                                    return myCheckBox(
                                      ...
                                      onChanged: (newValue) {
                                        dialogueSetState(() {
                                          ...
                                          // toggleTagMap.forEach(
                                          //     (index, isSelected) async {
                                          //   if (isSelected) {
                                          //     debugPrint(
                                          //         '$subIndex ${tags[subIndex]}');
                                          //   }
                                          // });
                                        });
                                        setState(() {});
...
class EditCoursePageOld extends StatelessWidget {
  const EditCoursePageOld({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<CourseCubit, CourseState>(
      listener: (context, state) {
        if (state is CourseSuccess && state.isUpdating) {
        ...
        if (state is CourseSuccess && state.updateSuccessMessage.isNotEmpty) {
          MevTechUtilities.successToast(context, state.updateSuccessMessage);
          if (context.canPop()) {
            context
              ..pop()
              ..pop();
  ...
  //     return Image.file(selectedImageFile!, width: 150, height: 150, fit: BoxFit.cover);
  //   } else if (imageUrl != null) {
  //     return Image.network(imageUrl!, width: 150, height: 150, fit: BoxFit.cover);
  //   } else {
  //     return Container(
  //       width: 150,
  //       height: 150,
